...
7. **Logowanie Google OAuth, Nowa Rola Kibica & Ekran Uzupełnienia Profilu**:
   - **Wykrywanie Nowego Konta i Eliminacja Race Condition (`contexts/AuthContext.tsx`, `NavigationGuard` w `app/_layout.tsx` i `complete_profile.tsx`)**: Wyeliminowano wyścig stanów (race condition) przy ponownym logowaniu Google OAuth. Wcześniej `user` ustawiał się przed pobraniem profilu z bazy, co powodowało niepotrzebne przekierowanie do `complete_profile`. Teraz `loading` pozostaje aktywny do momentu jednoczesnego pobrania sesji i profilu, `NavigationGuard` natychmiast przekierowuje uzupełniony profil do `(tabs)/news`, a ekran `complete_profile` automatycznie odsyła użytkowników posiadających już wybraną rolę.
