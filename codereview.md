...
7. **Logowanie Google OAuth, Nowa Rola Kibica & Ekran Uzupełnienia Profilu**:
   - **Wykrywanie Nowego Konta i Eliminacja Mrugania Ekranu (`contexts/AuthContext.tsx`, `NavigationGuard` w `app/_layout.tsx` i `complete_profile.tsx`)**: Wyeliminowano mruganie ekranu uzupełniania profilu (`complete_profile`) przy logowaniu. Flaga `loading` w `AuthContext` pozostaje aktywna podczas pobierania profilu z Supabase, a `NavigationGuard` przekierowuje do `complete_profile` wyłącznie wtedy, gdy profil jest w 100% załadowany i potwierdzono brak wybranej roli.
